...
void showscene (scene_t* scene, int scene_type, int menu) {
  ...
  /* Calculate the FPS. */
  long int t = (elapsed_total.tv_sec*1e6 + elapsed_total.tv_usec) 
             - (elapsed_last.tv_sec*1e6 + elapsed_last.tv_usec);
  fps = 1e6/t;
...
void draw_settings(scene_t *scene) {
  ...
  sprintf(buffer, "\
< %3d >     Maximum number of blocks to display at the same time.", 
          max_energy_blocks);
  ...
  while ((currOpt = (getopt_long(argc, argv, "d:h:v", stoptions, NULL))) != -1) {
    switch (currOpt) {
      case 'd':
        ...
        curr_data_dir = (char *) realloc(curr_data_dir, 
